						...
						if file[0] != "." and "word2vec_GIGA_Embeddings_yelp" in file and file[-4:] == ".txt":
							...
							if not vocab_words:
								...
								try:
									...
									polarities = label_propagate_continuous(embeddings, pos_seeds, neg_seeds, beta=0.99, nn=10,
									...
									if isinstance(word2vec_training_params_part,str):
										...
										print("Computed polarities using the random walk method and saved in {f}".format(f=os.path.join(save_dir,save_file_name)))
									else:
										print("Cannot estimate window,dimension part that was used for word2vec training from file name: {f}".format(f=file))
								except:
									print("Error inducing polarities for {cat} using random walk method".format(cat=yelp_category))
